...
export default function SettingsPage({ toggleColorMode: propToggle }) {

  const context = useContext(ThemeContext);

  // ✅ FIX: use real value only (no fallback to "light")
  const mode = context?.mode;

  const contextToggle = context?.toggleColorMode;

  // use prop if passed, otherwise fallback to context
  const toggleColorMode = propToggle || contextToggle || (() => {});

  const isDark = mode === "dark";

  return (
    <Box sx={{ p: 4, maxWidth: 800, margin: "auto", position: "relative" }}>
